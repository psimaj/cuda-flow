...
"""
This script generates a random flow network on a triangular grid
...
for i in range(1, n, 2):
    for j in range(1, n):
        cap = math.ceil(max_edge_cap * scaling_factor(i, j, n))
        print((i-1) * n + j, i * n + j + 1, random.randint(0, cap))
        print(i * n + j + 1, (i-1) * n + j, random.randint(0, cap))

for i in range(3, n+1, 2):
    for j in range(1, n):
        cap = math.ceil(max_edge_cap * scaling_factor(i, j, n))
        print((i-1) * n + j, (i-2) * n + j + 1, random.randint(0, cap))
        print((i-2) * n + j + 1, (i-1) * n + j, random.randint(0, cap))
